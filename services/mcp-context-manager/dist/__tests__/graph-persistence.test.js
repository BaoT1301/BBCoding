...
import { saveSnapshot, loadSnapshot, resolveSnapshotPath, } from "../graph/graph-persistence.js";
/**
 * Helper: create a minimal FileParseResult for testing.
 */
function makeFileResult(filePath, hash, symbols = [], relations = [], resolvedImports = []) {
    return {
        filePath,
...
describe("Graph Persistence", () => {
    ...
    describe("Snapshot round-trip", () => {
        it("save → load → verify node/edge counts match", async () => {
            ...
            store.upsertFileResult(makeFileResult("backend/app/main.py", "hash-main", [
                {
            ...
            store.upsertFileResult(makeFileResult("backend/app/database.py", "hash-db", [
                {
    ...
    describe("GraphStore methods", () => {
        ...
        it("importFromSnapshot restores graph state and rebuilds lookup maps", () => {
            ...
            store.upsertFileResult(makeFileResult("backend/app/database.py", "hash-db", [], [], ["backend/app/main.py"]));
            const exported = store.exportGraph();
            const hashes = store.getFileHashes();
...
//# sourceMappingURL=graph-persistence.test.js.map