...
import { resolveSnapshotPath, saveSnapshot, loadSnapshot, createDebouncedSave, } from "./graph/graph-persistence.js";
...
async function bootstrap() {
    ...
    if (snapshot) {
        ...
        console.error(`[live-context-manager] delta: reused=${delta.reused} reparsed=${delta.reparsed} deleted=${delta.deleted}`);
    ...
    try {
        ...
        console.error("[live-context-manager] failed to save snapshot:", err.message);
    }
    ...
    const watcher = new LiveFileWatcher({
        ...
        graphStore,
        onUpdate: (stats) => {
            console.error(`[live-context-manager] update files=${stats.files} reparsed=${stats.reparsed} dependents=${stats.dependents}`);
    ...
    registerContextTools(server, graphStore);
