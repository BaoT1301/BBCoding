...
describe("Property 7: Longest Prefix Cluster Assignment", () => {
    ...
    it("should assign files to the cluster with the longest matching path prefix", () => {
        ...
        fc.assert(fc.property(arbClusterConfig, arbPathSegment, // filename
    ...
    it("should always return the cluster with the longest matching prefix among all clusters", () => {
        ...
        const arbMultiClusterConfig = fc
            .tuple(arbPathSegment, fc.array(arbPathSegment, { minLength: 1, maxLength: 3 }), fc.array(arbHexColor, { minLength: 4, maxLength: 4 }), fc.array(arbLabel, { minLength: 4, maxLength: 4 }))
        ...
        fc.assert(fc.property(arbMultiClusterConfig, arbPathSegment, ({ clusters, paths }, filename) => {
            fs.writeFileSync(configPath, JSON.stringify({ clusters }));
            const loader = new ClusterConfigLoader(configPath);
            ...
            const matchingClusters = clusters.filter((c) => c.path && deepFile.startsWith(c.path));
            const expectedCluster = matchingClusters.reduce((best, c) => c.path.length > best.path.length ? c : best);
...
describe("Property 8: Default Cluster Fallback", () => {
    ...
    it("should return the default cluster for files that do not match any cluster prefix", () => {
        ...
        arbPathSegment, arbPathSegment, // filename
        fc.array(arbHexColor, { minLength: 3, maxLength: 3 }), fc.array(arbLabel, { minLength: 3, maxLength: 3 }))
            .filter(([clusterSegs, nonMatchSeg]) => {
            // Ensure the non-matching segment doesn't start with any cluster segment
            return !clusterSegs.some((seg) => nonMatchSeg.startsWith(seg) || seg.startsWith(nonMatchSeg));
        })
        ...
        fc.assert(fc.property(arbNonMatchingScenario, ({ clusters, unmatchedFilePath }) => {
            fs.writeFileSync(configPath, JSON.stringify({ clusters }));
            const loader = new ClusterConfigLoader(configPath);
    ...
    it("should return the default cluster for root-level files when all clusters have non-empty paths", () => {
        const arbRootFileScenario = fc
            .tuple(fc.uniqueArray(arbPathSegment, { minLength: 1, maxLength: 3, comparator: (a, b) => a === b }), arbPathSegment, // filename at root
        fc.array(arbHexColor, { minLength: 3, maxLength: 3 }), fc.array(arbLabel, { minLength: 3, maxLength: 3 }))
        ...
        fc.assert(fc.property(arbRootFileScenario, ({ clusters, rootFilePath }) => {
            fs.writeFileSync(configPath, JSON.stringify({ clusters }));
            const loader = new ClusterConfigLoader(configPath);
...
//# sourceMappingURL=cluster-assignment.property.test.js.map