...
// ---------------------------------------------------------------------------
function buildSSEMessage(event, data) {
    return `event: ${event}\ndata: ${JSON.stringify(data)}\n\n`;
}
function broadcastToClients(clients, event, data) {
...
const arbPayload = fc.oneof(arbTimestamp.map((ts) => ({ timestamp: ts })), fc.record({
...
describe("Property 45: SSE Keepalive Interval", () => {
    it("keepalive messages are valid SSE format: `event: keepalive\\ndata: {\"timestamp\":<number>}\\n\\n`", () => {
        fc.assert(fc.property(arbTimestamp, (timestamp) => {
    ...
    it("the keepalive interval constant is exactly 30,000ms", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 100 }), (_seed) => {
...
describe("Property 46: SSE Disconnect Resilience", () => {
    it("all non-throwing clients receive the message", () => {
        fc.assert(fc.property(arbClientSet, arbPayload, (clientDefs, payload) => {
            const clients = new Set(clientDefs.map((c) => ({
                ...c,
                receivedMessages: [],
            })));
    ...
    it("even when ALL clients fail, the function does not throw and the set is emptied", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 10 }), arbPayload, (clientCount, payload) => {
            // Create a set where ALL clients will fail
            const clients = new Set(Array.from({ length: clientCount }, (_, i) => ({
                id: i + 1,
                shouldFail: true,
                receivedMessages: [],
            })));
            const { threw, deliveredTo, removedClients } = broadcastToClients(clients, "keepalive", payload);
            // Assert: no throw
            expect(threw).toBe(false);
    ...
    it("healthy clients receive the exact SSE-formatted message (not corrupted)", () => {
        fc.assert(fc.property(arbClientSet, arbEventName, arbPayload, (clientDefs, event, payload) => {
            const clients = new Set(clientDefs.map((c) => ({
                ...c,
                receivedMessages: [],
            })));
            ...
            for (const client of clients) {
                ...
                expect(client.receivedMessages[0]).toMatch(new RegExp(`^event: ${event}\ndata: .+\n\n$`));
...
//# sourceMappingURL=sse-keepalive.property.test.js.map