...
describe("Property 6: Symbol Extraction Completeness", () => {
    ...
    it("should extract all Python function definitions", async () => {
        await fc.assert(fc.asyncProperty(fc.uniqueArray(arbPythonIdent, { minLength: 1, maxLength: 5, comparator: (a, b) => a === b }), async (funcNames) => {
    ...
    it("should extract all Python class definitions", async () => {
        await fc.assert(fc.asyncProperty(fc.uniqueArray(arbPythonIdent, { minLength: 1, maxLength: 5, comparator: (a, b) => a === b }), async (classNames) => {
            const tmpDir = makeTmpDir();
    ...
    it("should extract all Python import statements", async () => {
        await fc.assert(fc.asyncProperty(fc.uniqueArray(arbPythonModule, { minLength: 1, maxLength: 4, comparator: (a, b) => a === b }), async (moduleNames) => {
            const tmpDir = makeTmpDir();
    ...
    it("should extract all TypeScript import statements", async () => {
        await fc.assert(fc.asyncProperty(fc.tuple(fc.uniqueArray(arbTsIdent, { minLength: 1, maxLength: 4, comparator: (a, b) => a === b }), fc.uniqueArray(arbTsModulePath, { minLength: 1, maxLength: 4, comparator: (a, b) => a === b })).filter(([names, modules]) => names.length === modules.length), async ([names, modules]) => {
            const tmpDir = makeTmpDir();
            const imports = names.map((name, i) => `import { ${name} } from '${modules[i]}';`);
...
//# sourceMappingURL=symbol-extraction.property.test.js.map