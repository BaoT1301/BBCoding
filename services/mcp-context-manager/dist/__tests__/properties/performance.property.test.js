...
const arbFilePath = fc
    .tuple(fc.array(arbSegment, { minLength: 0, maxLength: 4 }), arbFileName)
 ...
 */
function buildGraphStore(filePaths, edgeCount) {
    ...
    const edgeTypes = ["imports", "calls", "reads", "writes", "references"];
...
describe("Property 47: No Hard Node Limits", () => {
    it("should NOT truncate the node array when no max_nodes param is provided (defaults to Infinity)", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 10000 }), (nodeCount) => {
            // Generate file paths
            const filePaths = [];
    ...
    it("should return all nodes when maxNodes is explicitly set to Infinity", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 5000 }), (nodeCount) => {
            const filePaths = [];
            for (let i = 0; i < nodeCount; i++) {
    ...
    it("should still respect explicit maxNodes when provided (backward compat)", () => {
        fc.assert(fc.property(fc.integer({ min: 10, max: 500 }), fc.integer({ min: 1, max: 9 }), (nodeCount, limit) => {
            const filePaths = [];
            for (let i = 0; i < nodeCount; i++) {
...
describe("Property 48: Geographic Mapping Performance", () => {
    it("should map 200 random file paths in < 1000ms", () => {
        fc.assert(fc.property(arbClusterPath, fc.array(arbFilePath, { minLength: 200, maxLength: 200 }), (clusterPath, relativePaths) => {
...
describe("Property 49: SSE Concurrent Client Support", () => {
    it("should deliver a message to all N clients in the set", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 10 }), fc.record({
    ...
    it("should handle mixed success/failure clients: deliver to healthy, remove failed", () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 10 }), fc.array(fc.boolean(), { minLength: 1, maxLength: 10 }), fc.record({
            type: fc.constantFrom("keepalive"),
            timestamp: fc.nat({ max: 2_000_000_000 }),
        }), (_, failureFlags, payload) => {
            const clients = new Set();
            const healthyReceived = [];
            let healthyCount = 0;
            let failCount = 0;
...
describe("Property 50: Initial Load Performance", () => {
    it("should build and export a graph with 500 nodes and 0–2000 edges within 30,000ms", () => {
        fc.assert(fc.property(fc.integer({ min: 0, max: 2000 }), (edgeCount) => {
    ...
    it("should export the graph in under 1 second for typical workloads (500 nodes, moderate edges)", () => {
        fc.assert(fc.property(fc.integer({ min: 50, max: 500 }), (edgeCount) => {
            const filePaths = [];
            for (let i = 0; i < 500; i++) {
...
//# sourceMappingURL=performance.property.test.js.map