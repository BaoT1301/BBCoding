...
const arbRelativeFilePath = fc
    .tuple(fc.array(arbSegment, { minLength: 0, maxLength: 5 }), arbFileName)
 ...
 */
const arbFileSet = fc
    .tuple(arbClusterPath, fc.array(arbRelativeFilePath, { minLength: 1, maxLength: 50 }))
...
describe("Property 9: Geographic Coordinate Bounds", () => {
    it("should return lat ∈ [-90, 90] and lng ∈ [-180, 180] for ANY generated file path within ANY cluster", () => {
        fc.assert(fc.property(arbFileSet, ({ clusterPath, filePaths }) => {
    ...
    it("should return valid bounds for single-file clusters", () => {
        fc.assert(fc.property(arbClusterPath, arbRelativeFilePath, (clusterPath, relativePath) => {
    ...
    it("should return valid bounds for deeply nested paths (up to 6 segments)", () => {
        const arbDeepPath = fc
            ...
            .chain((folders) => arbFileName.map((file) => [...folders, file].join("/")));
        fc.assert(fc.property(arbClusterPath, arbDeepPath, (clusterPath, deepRelative) => {
...
describe("Property 10: Hierarchical Clustering", () => {
    it("should place files in the same folder closer than files in a different sibling folder", () => {
        ...
        fc.assert(fc.property(arbHierarchicalCluster, ({ clusterPath, allFiles, sameFolder1, sameFolder2, differentFolder }) => {
...
describe("Property 11: Sibling Folder Distribution", () => {
    it("should assign non-overlapping coordinate regions to sibling folders", () => {
        ...
        const arbSiblingFolders = fc
            .tuple(arbClusterPath, fc.array(arbSegment, { minLength: 2, maxLength: 8 }).filter((folders) => {
            return new Set(folders).size === folders.length;
        ...
        fc.assert(fc.property(arbSiblingFolders, ({ clusterPath, folders, allFiles, folderFiles }) => {
...
//# sourceMappingURL=geographic-mapper.property.test.js.map