...
import { QueryTimeoutError, InvalidParamsError, NotFoundError, withTimeout, withRetry, paginate, parsePaginationParams, buildErrorResponse, errorStatusCode, } from "../utils/query-guards.js";
...
describe("withTimeout", () => {
    ...
    it("rejects with QueryTimeoutError when function exceeds timeout", async () => {
        await expect(withTimeout(async (_signal) => {
            // Simulate a slow operation
            await new Promise((resolve) => setTimeout(resolve, 500));
            return "too late";
        }, 50)).rejects.toThrow(QueryTimeoutError);
    });
...
describe("withRetry", () => {
    ...
    it("throws after exhausting all retries", async () => {
        ...
        await expect(withRetry(async () => {
            attempts++;
            throw new QueryTimeoutError(5000);
        }, 2, 10)).rejects.toThrow(QueryTimeoutError);
    ...
    it("does not retry non-timeout errors", async () => {
        ...
        }, 2, 10)).rejects.toThrow("not a timeout");
...
//# sourceMappingURL=query-guards.test.js.map