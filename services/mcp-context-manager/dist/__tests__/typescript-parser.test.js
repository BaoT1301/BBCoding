 ...
 */
async function parseSource(source, fileName = "test.ts") {
...
`);
            const methods = result.symbols.filter((s) => s.kind === "function" && s.qualifiedName.includes("UserService"));
...
`);
            const ctors = result.symbols.filter((s) => s.name === "constructor" && s.qualifiedName.includes("App"));
...
`);
            const methods = result.symbols.filter((s) => s.kind === "function" && s.qualifiedName.includes("Validator.validate"));
...
`);
            ...
            const helperCall = callRelations.find((r) => r.targetQualifiedName?.includes("helper"));
...
`);
            const callRelations = result.relations.filter((r) => r.type === "calls");
            const logCall = callRelations.find((r) => r.targetQualifiedName?.includes("log"));
...
`);
            const callRelations = result.relations.filter((r) => r.type === "calls");
            const unknownCall = callRelations.find((r) => r.targetQualifiedName?.includes("unknownFunction"));
...
`);
            ...
            const dogInherits = inheritsRelations.find((r) => r.targetQualifiedName?.includes("Animal"));
...
`);
            const inheritsRelations = result.relations.filter((r) => r.type === "inherits");
            expect(inheritsRelations.length).toBeGreaterThanOrEqual(1);
            const implRelation = inheritsRelations.find((r) => r.targetQualifiedName?.includes("Serializable"));
...
`);
            const accessors = result.symbols.filter((s) => s.name === "value" && s.kind === "function");
