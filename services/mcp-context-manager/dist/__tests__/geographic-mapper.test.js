...
describe("mapFileToCoordinates", () => {
    ...
    describe("Property 10: hierarchical clustering", () => {
        it("should place files in the same folder closer than files in different folders", () => {
            ...
            const sameFolderDist = Math.sqrt((mainCoords.lat - configCoords.lat) ** 2 + (mainCoords.lng - configCoords.lng) ** 2);
            // Distance between files in different folders (app/ vs tests/)
            const diffFolderDist = Math.sqrt((mainCoords.lat - testMainCoords.lat) ** 2 + (mainCoords.lng - testMainCoords.lng) ** 2);
        ...
        it("should place sibling files in the same subfolder closer than files in different subfolders", () => {
            ...
            const sameSubDist = Math.sqrt((userCoords.lat - jobCoords.lat) ** 2 + (userCoords.lng - jobCoords.lng) ** 2);
            // Distance between files in different subfolders (models/ vs routers/)
            const diffSubDist = Math.sqrt((userCoords.lat - routerAuthCoords.lat) ** 2 + (userCoords.lng - routerAuthCoords.lng) ** 2);
...
//# sourceMappingURL=geographic-mapper.test.js.map