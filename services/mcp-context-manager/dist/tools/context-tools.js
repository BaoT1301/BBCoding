...
export function registerContextTools(server, graphStore) {
    server.tool("get_function_context", "Get graph neighborhood around a function symbol.", {
    ...
    server.tool("get_file_dependents", "Find direct or transitive dependents/dependencies for a file.", {
    ...
    server.tool("get_symbol_references", "Resolve references to a qualified symbol.", {
    ...
    server.tool("get_callers", "Given a function name, return all functions that call it (reverse call graph). Supports depth parameter for transitive callers.", {
        function_name: z.string().min(1),
        file_path: z.string().optional(),
    ...
    server.tool("get_call_chain", "Given a function name and direction (upstream/downstream/both), return the full call chain as a directed subgraph of nodes and edges.", {
        function_name: z.string().min(1),
        file_path: z.string().optional(),
    ...
    server.tool("get_dead_code", "Find functions and classes with zero inbound calls/instantiations (potential dead code). Supports filtering by file pattern, language, and symbol kind.", {
    ...
    server.tool("get_hotspots", "Return the top-N most-referenced symbols in the codebase (highest fan-in). Supports filtering by symbol kind, language, and file pattern. Useful for identifying critical code paths and high-risk symbols.", {
    ...
    server.tool("get_impact_analysis", "Given a file path, compute the transitive closure of all files and symbols that would be affected by a change to that file. Returns affected files, affected symbols, risk score, and suggested test files.", {
        file_path: z.string().min(1),
    ...
    server.tool("export_dependency_graph", "Export graph slice for visualization libraries such as D3.js or React Flow.", {
    ...
    server.tool("get_module_coupling", "Compute coupling metrics between two file paths. Returns shared imports, shared symbols, direct edges, transitive edges, and a normalized coupling score (0.0–1.0).", {
    ...
    server.tool("search_symbols", "Fuzzy search across all symbols in the graph by name, kind, and file path. Returns ranked results with match scores. Supports regex patterns for advanced queries.", {
    ...
    server.tool("get_complexity_metrics", "Compute per-symbol complexity metrics: fan-in (inbound edges), fan-out (outbound edges), and max call-chain depth. Supports filtering by file path, symbol kind, and language. Results sorted by total complexity descending.", {
        file_path: z.string().optional(),
    ...
    server.tool("get_circular_dependencies", "Detect circular import chains in the codebase. Uses DFS-based cycle detection on the file-level import graph. Supports filtering by file pattern and language.", {
        file_pattern: z.string().optional(),
        language: z.enum(["python", "typescript"]).optional(),
    ...
    server.tool("get_change_risk", "Given a set of changed file paths (e.g. from a git diff), predict which tests should run and which areas of the codebase are highest risk. Aggregates impact analysis across multiple files and cross-references with hotspots.", {
    ...
    server.tool("get_class_hierarchy", "Get the class inheritance hierarchy (ancestors and/or descendants) for a given class. Traverses 'inherits' edges to build the full hierarchy tree.", {
