...
describe("Property 5: File Extension Filtering", () => {
    const allowedExtensions = [".py", ".ts", ".tsx", ".js", ".jsx"];
    ...
    const arbDirPrefix = fc
        .array(fc.stringMatching(/^[a-zA-Z_][a-zA-Z0-9_-]{0,15}$/), { minLength: 0, maxLength: 4 })
    ...
    it("should return 'python' or 'typescript' for allowed extensions", () => {
        ...
        const arbAllowedPath = fc.tuple(arbDirPrefix, arbFilenameStem, arbAllowedExt).map(([dir, stem, ext]) => `${dir}${stem}${ext}`);
        fc.assert(fc.property(arbAllowedPath, (filePath) => {
    ...
    it("should return null for disallowed extensions", () => {
        ...
        const arbDisallowedPath = fc.tuple(arbDirPrefix, arbFilenameStem, arbDisallowedExt).map(([dir, stem, ext]) => `${dir}${stem}${ext}`);
        fc.assert(fc.property(arbDisallowedPath, (filePath) => {
            const language = detectLanguage(filePath);
    ...
    it("should correctly partition all generated paths by extension type", () => {
        ...
        const arbAnyPath = fc.tuple(arbDirPrefix, arbFilenameStem, arbAnyExt).map(([dir, stem, ext]) => `${dir}${stem}${ext}`);
        fc.assert(fc.property(arbAnyPath, (filePath) => {
            const language = detectLanguage(filePath);
...
//# sourceMappingURL=file-filtering.property.test.js.map