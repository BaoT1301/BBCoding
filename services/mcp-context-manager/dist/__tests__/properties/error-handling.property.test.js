...
describe("Property 61: Parse Error Recovery", () => {
    ...
    it("parseTypeScriptFile returns a valid FileParseResult for any file content (including invalid syntax)", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 0, maxLength: 500 }), async (content) => {
    ...
    it("parseTypeScriptFile result always includes filePath and language regardless of content", async () => {
        ...
        fc.oneof(fc.string({ minLength: 0, maxLength: 200 }), 
        // Binary-like content
        fc.uint8Array({ minLength: 1, maxLength: 100 }).map((arr) => String.fromCharCode(...arr)), 
        // Partial/broken syntax
        fc.constantFrom("function {{{", "import from ;; ;;", "class extends {}}}", "const = = = ;", "export default (", "let x: = 42;", "async function* () { yield yield yield")), async (content) => {
            const tmpDir = makeTmpDir();
    ...
    it("parsePythonFile populates parseErrors array when tree-sitter encounters a parse failure", async () => {
        ...
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 300 }), async (content) => {
            const tmpDir = makeTmpDir();
    ...
    it("IncrementalIndexer continues processing subsequent files after a parse error in one file", async () => {
        ...
        await fc.assert(fc.asyncProperty(fc.tuple(fc.string({ minLength: 1, maxLength: 200 }), // garbage content
        fc.constantFrom("def hello():\n    pass\n", "class Foo:\n    pass\n", "import os\n")), async ([garbageContent, validContent]) => {
            const tmpDir = makeTmpDir();
            // First file: garbage
...
describe("Property 63: Malformed Config Error Response", () => {
    ...
    it("falls back to DEFAULT_CLUSTER for any invalid JSON content", () => {
        ...
        fc.oneof(fc.string({ minLength: 1, maxLength: 200 }).filter((s) => {
            try {
                JSON.parse(s);
                return false;
            }
        ...
        }), fc.constantFrom("not json at all", "{incomplete", "[1, 2, 3", "{'single': 'quotes'}", "")), (invalidJson) => {
            const tmpDir = makeTmpDir();
    ...
    it("falls back to DEFAULT_CLUSTER for valid JSON with invalid schema", () => {
        ...
        fc.jsonValue().map((v) => JSON.stringify(v))), (invalidSchemaJson) => {
            const tmpDir = makeTmpDir();
            const configPath = path.join(tmpDir, "cluster-config.json");
    ...
    it("falls back to DEFAULT_CLUSTER when config contains absolute paths", () => {
        fc.assert(fc.property(fc.tuple(fc.stringMatching(/^[a-z][a-z0-9\-]{0,10}$/), fc.stringMatching(/^[a-z][a-z0-9\-]{0,10}$/), fc.stringMatching(/^[0-9A-Fa-f]{6}$/)), ([id, label, colorHex]) => {
            const tmpDir = makeTmpDir();
            const configPath = path.join(tmpDir, "cluster-config.json");
            const config = {
                clusters: [
    ...
    it("falls back to DEFAULT_CLUSTER when config file does not exist", () => {
        fc.assert(fc.property(fc.stringMatching(/^[a-z]{5,15}$/), (randomName) => {
    ...
    it("getClusters() never throws regardless of config content", () => {
        fc.assert(fc.property(fc.string({ minLength: 0, maxLength: 500 }), (content) => {
            const tmpDir = makeTmpDir();
            const configPath = path.join(tmpDir, "cluster-config.json");
...
describe("Property 64: Fallback Node Positioning", () => {
    it("returns { lat: 0, lng: 0 } for file paths that don't match any cluster path prefix", () => {
        ...
        fc.tuple(fc.stringMatching(/^[a-z]{3,10}$/), fc.stringMatching(/^[a-z]{3,10}$/), fc.constantFrom(".ts", ".py", ".js")).map(([dir, name, ext]) => `${dir}/${name}${ext}`), (filePath) => {
    ...
    it("returns { lat: 0, lng: 0 } when allFilePaths is empty", () => {
        fc.assert(fc.property(fc.tuple(fc.stringMatching(/^[a-z]{3,10}$/), fc.stringMatching(/^[a-z]{3,10}$/)).map(([dir, name]) => `${dir}/${name}.ts`), fc.stringMatching(/^[a-z]{3,10}\/$/).map((s) => s), (filePath, clusterPath) => {
            const allFilePaths = [];
            const coords = mapFileToCoordinates(filePath, clusterPath, allFilePaths);
    ...
    it("never crashes and always returns valid coordinates for any input", () => {
        fc.assert(fc.property(fc.string({ minLength: 0, maxLength: 100 }), fc.string({ minLength: 0, maxLength: 50 }), fc.array(fc.string({ minLength: 0, maxLength: 100 }), { minLength: 0, maxLength: 20 }), (filePath, clusterPath, allFilePaths) => {
...
//# sourceMappingURL=error-handling.property.test.js.map