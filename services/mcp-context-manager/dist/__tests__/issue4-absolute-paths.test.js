 ...
 */
function makeFileResult(filePath, language, imports = []) {
    return {
        filePath,
        language,
        hash: `hash-${filePath}`,
...
describe("Issue #4: Absolute Docker path → relative path stripping for cluster matching", () => {
    ...
    beforeEach(() => {
        ...
        clusterConfig = {
            ...
            getClusterForFile: vi.fn().mockImplementation((filePath) => {
    ...
    it("should assign correct clusterId when node filePaths are absolute Docker paths", async () => {
        ...
        try {
            ...
            const clusterIds = fileNodes.map((n) => n.clusterId);
            ...
            const backendNode = fileNodes.find((n) => n.filePath?.includes("backend/"));
            expect(backendNode?.clusterId).toBe("backend");
            const frontendNode = fileNodes.find((n) => n.filePath?.includes("frontend/"));
            expect(frontendNode?.clusterId).toBe("frontend");
            const servicesNode = fileNodes.find((n) => n.filePath?.includes("services/"));
    ...
    it("should detect cross-cluster edges when filePaths are absolute Docker paths", async () => {
        ...
        try {
            ...
            const crossEdges = data.edges.filter((e) => e.isCrossCluster === true);
    ...
    it("should verify getClusterForFile receives relative paths, not absolute", async () => {
        ...
        try {
            ...
            const calls = clusterConfig.getClusterForFile.mock.calls;
...
//# sourceMappingURL=issue4-absolute-paths.test.js.map